...
jQuery(function($) {
  ...
  //TODO write a single .on('click') handler to reset the localStorage for that data-target
  $(".reset").on('click', function  (){
    //Store our button as a jQuery object
    var $this = $(this),
      targetVar = $this.data('target');
    console.log('Click this target variable', targetVar, 'click this target-id', $this.data('target-id'));
    targetVar = 0;
    switch (targetVar) {
      case 'wins':
        wins = 0;
    ...
    window.localStorage.removeItem($this.data('target'));
    $('#' + $this.data('target-id')).html(0);
  });
});